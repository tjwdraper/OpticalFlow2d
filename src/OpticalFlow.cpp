...
void OpticalFlow::display_registration_parameters(const float alpha) const {
    mexPrintf("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n");
...
OpticalFlow::OpticalFlow(const dim dimin, const int nscales, const int* niter, const float alpha) {
    ...
    // Solver object
    this->solver = new OpticalFlowSolver*[nscales + 1];
    for (int s = nscales; s >= 0; s--) {
        this->solver[s] = new OpticalFlowSolver(this->dimin[s], alpha);
    }
...
// Getters and setters
void OpticalFlow::set_reference_image(const Image& im) {
...
void OpticalFlow::set_moving_image(const Image& im) {
    // Set the image at the largest resolution level
    *(this->Imov[0]) = im;
...
Motion* OpticalFlow::get_estimated_motion() const {
    return this->motion[0];
}

// Copy the estimated motion 
void OpticalFlow::copy_estimated_motion(Motion& mo) const {
    mo = *this->motion[0];
}

// Estimate motion
void OpticalFlow::estimate_motion_at_current_resolution(Motion* motion, 
...
void OpticalFlow::estimate_motion() {    
    // Multiresolution pyramid
    for (int s = this->nscales; s >= 0; s--) {
        mexPrintf("Current resolution level: %d - dimensions (%d %d) - size (%d) - niter (%d)\n", 
