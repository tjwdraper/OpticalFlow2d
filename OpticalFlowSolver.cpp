...
void OpticalFlowSolver::get_image_gradients(const Image* Iref, const Image* Imov) {
...
// Do one iteration
void OpticalFlowSolver::get_quasi_laplacian(const Motion* motion) {
...
void OpticalFlowSolver::horn_schunck_iteration(Motion *motion) {
    // Get the dimensions and step size of the motion field
    const dim& dimin = this->dimin;
    const dim& step = this->step;
...
void OpticalFlowSolver::get_update(Motion *motion) {
